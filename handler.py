...
def _is_off_topic(text_en):
    """
    Detect if user input is off-topic, inappropriate, or vulgar.
    Returns: 'vulgar', 'complaint', 'off_topic', or None (valid input).
    """
    text = text_en.strip().lower()

    # Skip very short inputs (single words like "yes", "no" are not off-topic)
    if len(text.split()) <= 2:
        return None

    # Vulgar / abusive words
    vulgar_words = [
        "fuck", "shit", "ass", "bitch", "bastard", "damn", "idiot", "stupid",
    ...
    complaint_phrases = [
        "not a good bot", "not good", "useless", "waste of time",
        "terrible", "worst", "not satisfied", "unhelpful", "not helpful",
        "don't understand", "doesn't understand", "not working",
        "bad service", "bad bot", "poor service", "disappointed",
        "talk to a real person", "talk to human", "real agent",
        "not resolve", "didn't resolve", "can't help", "cannot help",
        "you suck", "hopeless", "pathetic", "rubbish",
    ]
    ...
    off_topic_phrases = [
        "date with me", "come with me", "marry me", "i love you",
        "what is your name", "how old are you", "where do you live",
        "tell me a joke", "sing a song", "play music",
        "what is the weather", "book a flight", "order food", "order pizza",
        "who is the president", "who is the prime minister",
        "what is the capital", "solve this math",
        "help me with homework", "write an essay", "code for me",
        "are you human", "are you a robot", "are you real",
        "what can you do", "who made you", "who created you",
    ]
    if any(p in text for p in off_topic_phrases):
        return "off_topic"

    # General off-topic detection: no property-related keywords at all
    property_keywords = [
        "flat", "apartment", "house", "villa", "bungalow", "plot", "studio",
        "bhk", "bedroom", "property", "budget", "lakh", "crore", "lac",
        "mumbai", "gurgaon", "hyderabad", "kolkata", "pune", "delhi",
...
def lambda_handler(event, context):
    ...
    # ── Off-topic / Inappropriate input handling ───────────────
    if user_text_en:
        off_topic_type = _is_off_topic(user_text_en)
        if off_topic_type:
            print(f"[OFF-TOPIC] type={off_topic_type} | '{user_text_en[:50]}'")
            if off_topic_type == "vulgar":
                return reply(
                    "I understand your frustration. Let me help you find the right property. "
                    "What type of property are you looking for?"
                )
            elif off_topic_type == "complaint":
                return reply(
                    "I'm sorry for the inconvenience. I want to help you better. "
                    "Would you like to continue with property search, or shall I connect you with our team?"
                )
            else:  # off_topic
                return reply(
                    "I appreciate your message, but I can only help with property search. "
                    "Could we get back to finding your ideal property?"
                )
